...
$(document).ready(function(){
		...
		if ( container.attr('id') != 'workarea' ) {
			...
			allChildren.each(function(idx,val){
				if ($(val).html().indexOf("&amp;") != -1) {
					numChildren+=(mainStr.split("&amp;").length - 1);
					//numChildren+=2;
				} else {
					numChildren++;
				}
	...
	*********************************************************/
	...
	$("#menuPrint").live('click',function() {
		...
		doc.setFontSize(8);
		doc.text(2, 16, "http://stefslon.github.io/sorty");
		doc.setFontSize(10);
		...
		try {
			...
			// Fix: https://stackoverflow.com/questions/45493234/jspdf-not-allowed-to-navigate-top-frame-to-data-url
